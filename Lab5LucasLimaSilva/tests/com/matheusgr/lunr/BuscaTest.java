...
class BuscaTest extends BaseTest {
	...
	/**
	 * Teste de busca avançada com um metadado
	 * 
	 * Eu inseri mais um documento para os testes, por isso o valor de total documentos passou de 2 para 5.
	 */
	@Test
	void testBuscaAvancada() {
		...
		assertEquals(5, busca.length, "Todos os documentos de texto");

		Set<String> ids = Stream.of(busca).map(DocumentoDTO::getId).collect(Collectors.toSet());
		Set<String> expectedIds = Stream.of(new String[] {TEXTO1_ID, TEXTO2_ID, TEXTO3_ID, TEXTO4_ID, TEXTO5_ID}).collect(Collectors.toSet());
		assertEquals(expectedIds, ids);
	}
	
	/**
	 * Teste de busca avanaçada com dois metadados.
	 */
	@Test
