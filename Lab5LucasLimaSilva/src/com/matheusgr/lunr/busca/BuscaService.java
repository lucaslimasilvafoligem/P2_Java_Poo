...
public class BuscaService {
	...
	public DocumentoDTO[] busca(BuscaAvancada buscaAvancada) {
		...
		DocumentoDTO[] documentos = converte(respostaDocumento);
	...
	/**
	 * Converte um map em um tipo dicumentoDTO
	 * 
	 * @param respostaDocumento
	 * @return Retorna uma conversão de map para documentodto
	 */
	private DocumentoDTO[] converte(Map<Documento, Integer> respostaDocumento) {
		return respostaDocumento.entrySet().stream()
				.map(Entry::getKey)
				.map(DocumentoDTO::new)
				.collect(Collectors.toList())
